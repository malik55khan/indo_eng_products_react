...
const Enquiries = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    API.getEnquiries().then((response: AxiosResponse<any, any>) => {
      if (response?.data?.data) setEnquiryList(response.data.data);
      setLoading(false);
  ...
  return (
    <>
      <Breadcrumb pageName="Machine Enquiries" />
      {showModal && <ViewEnquiryModal setShowModal={setShowModal} enquiry={enquiry} />}
      <div className="flex flex-col gap-10">
        ...
        <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          ...
          {enquiryList.map((enquiry: EnquiryType) => <div key={enquiry.id} className="grid grid-cols-6 border-t border-stroke py-4.5 px-4 dark:border-strokedark sm:grid-cols-8 md:px-6 2xl:px-7.5">
            <div className="col-span-2 flex items-center">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                <p className="text-sm text-black dark:text-white">{enquiry.company_name}</p>
              </div>
            </div>
            <div className="col-span-2 hidden items-center sm:flex">
              <p className="text-sm text-black dark:text-white">{enquiry.designation}</p>
            </div>
            <div className="col-span-1 flex items-center">
              <p className="text-sm text-black dark:text-white">{enquiry.phone}</p>
            </div>
            <div className="col-span-1 flex items-center">
              <p className="text-sm text-black dark:text-white">{enquiry.email}</p>
            </div>
            <div className="col-span-1 flex items-center">
              <p className="text-sm text-black dark:text-white">{new Date(enquiry.created_at).toLocaleString()}</p>
            </div>
            <div className="col-span-1 flex items-center">
              <div className="flex items-center space-x-3.5">
                <button className="hover:text-primary" onClick={() => { setShowModal(true); setEnquiry(enquiry); }}>
